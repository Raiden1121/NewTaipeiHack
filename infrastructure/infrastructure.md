...
⚠️ `terraform apply` 會 build 並 push 這支 Lambda 的 container image，所以執行 apply 的機器**需要 Docker daemon 正在執行**（本機是 Docker Desktop）。image tag 取自 `data-pipeline/src`、`data-pipeline/config`、`requirements.txt`、`modules/analytics_lambda/lambda/` 與 `Dockerfile` 的內容雜湊，原始碼沒變就不會重 build，也不會重新部署 Lambda。
