...
def _demo():
    """Router/envelope/Decimal-encoding self-check against a fake in-memory
    table — no AWS credentials needed. Doesn't exercise real DynamoDB access;
    that's covered by hitting the deployed API directly."""
    import sys

    fake_table = {
        ("META", "MANIFEST"): {
            "snapshot_id": "demo", "generated_at": "2026-01-01T00:00:00Z",
            "calculation_version": "1", "districts_count": 1,
            "time_policy": {"annual_years_roc": [114], "election_years_roc": [111]},
            "quality": {"status": "observed"},
            "warnings": [],
        },
        ("DASHBOARD", "KPIS"): {
            "kpis": {"cityYouthPopulation": Decimal("845938")},
            "availability": {"opportunityIndex": "available"},
        },
        ("DASHBOARD", "POLICY"): {
            "currentBudget": Decimal("196153"), "budgetTrend": [], "executionRate": None,
        },
        ("DASHBOARD", "SERVICE_COVERAGE"): {"value": Decimal("49.23"), "status": "partial"},
        ("DASHBOARD", "ELECTIONS"): {"city_councilor_t1_citywide": []},
        ("DASHBOARD", "POPULATION_TREND"): {
            "years": [
                {"year_roc": 113, "city": {"youth_population": Decimal("860000")}},
                {"year_roc": 114, "city": {"youth_population": Decimal("845938")}},
            ]
        },
        ("DASHBOARD", "FERTILITY_TREND"): {"years": []},
        ("DASHBOARD", "DISTRICTS"): {"districts": [{"district_id": "65000010", "district_name": "板橋區"}]},
        ("DISTRICT#65000010", "SUMMARY"): {
            "district_id": "65000010", "district_name": "板橋區", "opportunityIndex": Decimal("5.17"),
        },
        ("ANALYSIS#policy-outcomes", "DATA"): {
            "analysis_id": "policy-outcomes", "wageTrend": [],
            "currentWageGrowth": Decimal("2.8"), "currentPopGrowth": Decimal("-1.969"),
            "desiredDirection": {"wageGrowth": "up", "populationChange": "up"},
        },
    }

    module = sys.modules[__name__]
    original_get_items = module._get_items
    module._get_items = lambda keys: {k: fake_table[k] for k in keys if k in fake_table}
    try:
        ...
        assert json.loads(r["body"])["data"]["districts_count"] == 1

        r = call("GET", "/api/v1/dashboard/overview")
        body = json.loads(r["body"])["data"]
        assert body["kpis"]["cityYouthPopulation"] == 845938  # Decimal(int) -> int
        ...
        assert json.loads(r["body"])["request_id"] == "unknown"

        r = call("GET", "/api/v1/analyses/policy-outcomes")
        data = json.loads(r["body"])["data"]
        assert data["currentWageGrowth"] == 2.8  # Decimal(non-int) -> float
        assert data["populationTrend"][1]["yoy"] == round((845938 - 860000) / 860000 * 100, 2)

        r = call("GET", "/api/v1/analyses/does-not-exist")
        assert r["statusCode"] == 404
        assert json.loads(r["body"])["error"]["code"] == "ANALYSIS_NOT_FOUND"

        module._get_items = lambda keys: {}
        r = call("GET", "/api/v1/catalog")
        assert r["statusCode"] == 503
        assert json.loads(r["body"])["error"]["code"] == "SNAPSHOT_UNAVAILABLE"
    finally:
        module._get_items = original_get_items
